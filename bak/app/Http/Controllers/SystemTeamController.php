...
class SystemTeamController extends Controller
{
    ...
    public function list(Request $request) {
        ...
        $list = $service->getTeamTree(0, true);
        $data['total'] = 1000;
        $data['list'] = $list;
    ...
    public function edit(Request $request) {
        ...
        if (isset($params['id'])) {
            ...
            $service = new UserService();
            $subTeams = $service->getTeams($params['id']);
            if (in_array($params['parent_id'], $subTeams)) {
                return $this->apiReturn(static::ERROR, [], '不能把下属团队设置为上级团队');
            }
        }
        $model->name = $params['name'];
        $model->manager_id = intval($params['manager_id']);
    ...
    public function info(Request $request) {
        ...
        if (isset($params['id'])) {
            ...
            $model['parent_id'] = empty($model['parent_id']) ? "" : intval($model['parent_id']);
        }
        $usermodel = new SystemUser();
        ...
        $allTeamListSelect = $model->getAllTeam();
        $allTeamListSelect = app(SelectService::class)->genSelect($allTeamListSelect, 'id', 'name'); // 转成前端的select
        return $this->apiReturn(static::OK, ['teamInfo'=>$model, 'allUserListSelect' => $allUserListSelect, 'allTeamListSelect' => $allTeamListSelect]);
    }
