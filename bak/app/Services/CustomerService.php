...
{
    const KEY = "SDF1RLEOCNBTFOUM";
    const IV = "UNC16IOSCDF1BQOO";
    ...
    public function genCustomByExcelRow($row, $dict) {
        ...
        $returnData['custom']['city'] = intval(array_flip($dict[SystemDict::TYPE_CITY])[$row[3]]);
        ...
        $returnData['custom']['remark'] = $row[13];
        $returnData['custom']['star'] = intval($row[14]);
	Log::info("IN1686510039 新用户: " . $row[15]);
	Log::info("IN1686510039 新用户: " . json_encode(array_flip($dict[SystemDict::TYPE_FOLLOW])));
        if (!empty($row[15]) && empty(array_flip($dict[SystemDict::TYPE_FOLLOW])[$row[15]])) {
            $returnData['error'][] = '找不到对应的跟进状态';
        }
        $returnData['custom']['follow_status'] = intval(array_flip($dict[SystemDict::TYPE_FOLLOW])[$row[15]]);
        $model = new SystemUser();
        $userNames = array_column($model->getAllUser()->toArray(), 'id', 'name');
        if (!empty($row[16]) && empty($userNames[$row[16]])) {
            $returnData['error'][] = '找不到对应的跟进人';
        }
        $returnData['custom']['follow_user_id'] = intval($userNames[$row[16]]);

	$channel = new Channel();
        $channels = array_column($channel->get()->toArray(), 'id', 'name');
        if (!empty($row[17]) && empty($channels[$row[17]])) {
            $returnData['error'][] = '找不到对应的渠道';
        }
        $returnData['custom']['source'] = intval($channels[$row[17]]);
    ...
    public function encrypt($str) {
        return bin2hex(openssl_encrypt($str, 'aes-256-cbc', static::KEY, OPENSSL_RAW_DATA, static::IV));
    }
    public function decrypt($str) {
        if (empty($str)) {
            return "";
        }
        return @openssl_decrypt(hex2bin($str), 'aes-256-cbc', static::KEY, OPENSSL_RAW_DATA, static::IV);
    }
