...
class Customer extends Model
{
    ...
    private function _createWhere($params) {
        ...
        if (isset($params['remark']) && $params['remark'] !== "") {
            $query = $query->where('remark', 'like', '%'.$params['remark'].'%');
        }
        if (isset($params['mobile']) && $params['mobile'] !== "") {
            $mobileJiami = app(CustomerService::class)->encrypt($params['mobile']);
            $query = $query->where('mobile_jiami',  $mobileJiami);
        }
        if (isset($params['city']) && $params['city'] !== "") {
            $query = $query->where('city',  $params['city']);
        }
        if (isset($params['followUserId']) && $params['followUserId'] !== ""  && in_array($params['type'], ['CustomerImportList', 'CustomerInnerList', 'CustomerNewList', 'CustomerList', 'MyCustomerList'])) {
            $query = $query->where('follow_user_id',  $params['followUserId']);
        }
        if (in_array($params['type'], ['MyCustomerList'])) {
            $query = $query->whereRaw('follow_time > assign_time');
        }
        if (isset($params['lastFollowUserId']) && $params['lastFollowUserId'] !== ""  && in_array($params['type'], ['CustomerPool'])) {
            $query = $query->where('last_follow_user_id',  $params['lastFollowUserId']);
        }
        ...
        if (isset($params['maxFt']) && $params['maxFt'] !== "") {
            $query = $query->where('follow_times', '<=', $params['maxFt']);
        }

        if (isset($params['minFt']) && $params['minFt'] !== "") {
            $query = $query->where('follow_times', '>=', $params['minFt']);
        }
        ...
        if (isset($params['star']) && $params['star'] !== "") {
            ...
            } else if ($params['star'] == 0) {
                $query = $query->where('star', '=', -1);
            } else if ($params['star'] == 9) {
                $query = $query->whereIn('star', [2,3]);
            } else if ($params['star'] == 10) {
                $query = $query->whereIn('star', [4,5]);
            } else {
        ...
        if (isset($params['notFollow']) && $params['notFollow']!= '') {
            $query = $query->where('assign_time', '<', time() - ($params['notFollow'] * 24 *3600))
        ...
        if (isset($params['giveupTime']) && !empty($params['giveupTime']) && is_array($params['giveupTime']) ) {
            $query = $query->where('giveup_time',  '>', strtotime($params['giveupTime'][0]));
            $query = $query->where('giveup_time',  '<', strtotime($params['giveupTime'][1]));
        }
    ...
    public function analyst($params) {
        $sql = "select a.`after` as follow_user_id,star,count(distinct b.id) as cnt from customer_log a, customer b where a.type = 15 and a.customer_id = b.id ";
        $whereArr = [];
        if ($params['team_id']) {
            $userService = new UserService();
            $teamids = $userService->getTeams($params['team_id']);
            ...
            if (empty($userIds)) {
                ...
                $sql .= " and a.`after` in ( ".implode(",", $tmp)." ) ";
            }
        }
        if ($params['users']) {
            $sql .= " and a.`after` in ( ".implode(",", $params['users'])." ) ";
        }
        if (isset($params['followUserId']) && $params['followUserId'] !== "") {
            $sql .= " and a.`after` =  ?  ";
            $whereArr[] = $params['followUserId'];
        }
        if (isset($params['times']) && !empty($params['times']) && is_array($params['times']) ) {
            $sql .= " and a.create_time > ? "; 
            $sql .= " and a.create_time <= ? "; 
            $whereArr[] = $params['times'][0];
            $whereArr[] = $params['times'][1] . " 23:59:59";
        }
        $sql .= " group by `after`, star";
        return json_decode(json_encode(app('db')->select($sql, $whereArr)), true);
    }

    
    public function channelanalyst($params) {
        $query = $this;
        if (isset($params['source']) && !empty($params['source'])) {
            if (!empty($params['source'][0]) || count($params['source']) > 1) {
                $query = $query->whereIn('source',  $params['source']);
            }
        }
        if ($params['timeType'] == 1) {
            $query = $query->where('create_time', '>', date('Y-m-d 00:00:00'));
        ...
        if ($params['timeType'] == 8) {
            $query = $query->where('create_time', '>', date("Y-01-01 00:00:00"));
        }
        if ($params['timeType'] == 6) {
            $query = $query->where('create_time', '>', $params['times'][0]. ' 00:00:00');
            $query = $query->where('create_time', '<', $params['times'][1]. " 23:59:59");
        }
        return $query->whereRaw('status = 1 and source > 0')->groupby($params['type'])->selectRaw($params['type'].', count(*) as cnt')->get();
    }

    
    public function getUserCount($userId) {
        $list = $this->where('follow_user_id', $userId)->count();
        return $list;
    }
