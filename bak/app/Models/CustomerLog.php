...
class CustomerLog extends Model
{
    ...
    public function getCountByUserIdAndTimeIgnore($userId, $type, $startTime, $endTime) {
        $sql = "SELECT  count(distinct cl.customer_id) as cnt, max(cl.id) as id FROM customer_log cl left join customer c on  cl.customer_id = c.id where cl.type = ?
            and cl.`after` = ? and cl.create_time >= ? and cl.create_time < ? and c.source != 6";
        return app('db')->select($sql, [$type, $userId, $startTime, $endTime]);
    }

    public function getCountByUserIdAndTimeWithSource($userId, $type, $startTime, $endTime) {
        $sql = "SELECT  count(distinct cl.customer_id) as cnt, max(cl.id) as id FROM customer_log cl left join customer c on  cl.customer_id = c.id where cl.type = ?
            and cl.`after` = ? and cl.create_time >= ? and cl.create_time < ? and c.source = 6";
        return app('db')->select($sql, [$type, $userId, $startTime, $endTime]);
    }
    ...
    public function getTodayNew($userId) {
        $list = $this->where('after', $userId)->where('type', static::TYPE_ASSIGN_NEW)->where('create_time', '>=', date('Y-m-d'))->count();
        return $list;
    }

    
    public function getTodayAgain($userId) {
        $list = $this->where('after', $userId)->whereIn('type', [static::TYPE_GET, static::TYPE_ASSIGN])->where('create_time', '>=', date('Y-m-d'))->count();
        return $list;
    }
}
