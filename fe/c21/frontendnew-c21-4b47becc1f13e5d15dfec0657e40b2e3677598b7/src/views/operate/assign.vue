    ...
    <a-card class="general-card" style="padding-top:30px">
        ...
        <a-col :span="16">
          <a-space>
            <a-space  :size="18">
              ...
              <span>系统当日可接收新数据数量<font color="red">{{newlimit}}</font>条, 已接收<font color="red">{{newnow}}</font>条,剩余可接收数据<font color="red">{{newlimit-newnow}}</font>条;</span>
      ...
      <a-table row-key="id" :loading="loading" :pagination="pagination" :data="renderData" :bordered="false" label-align="center" @page-change="onPageChange" >
        <template #columns>
          ...
          <a-table-column title="内部流转分配上限" data-index="inner" />
          <a-table-column title="新数据上限" data-index="new" />
          <a-table-column title="操作" data-index="operations" :width="324">
